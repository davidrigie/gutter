...
| Marker inside a code block or inline code | Not a comment. Literal text — don't parse. |
...
### File 2: `*.comments.json` — Thread Data
...
### File 3: `*.comments.md` — Generated Companion
...
**Dave** — Feb 13, 2026 10:30 AM
Should we rephrase this? "Long-term growth" is vague.

**Sarah** — Feb 13, 2026 10:35 AM
How about "sustainable revenue growth"?

✅ *Resolved by Dave — Feb 13, 2026 10:42 AM*

---

> **[c2]** on "quick wins"

**Dave** — Feb 13, 2026 11:00 AM
...
- Resolution status: `✅ *Resolved by {author} — {timestamp}*` or `🟡 *Open*`
...
- Parse: MD string → editor document model
- Serialize: editor document model → MD string
- **Requirement:** `serialize(parse(md))` must be functionally equivalent to `md` — no content loss, no format corruption. Whitespace normalization is acceptable.
- Inline HTML (including comment markers) must survive the round trip exactly

**Test fixture — this file must round-trip cleanly:**
...
*(Note: the spaces in the fenced code block delimiters above are for escaping only — the actual test fixture should use real triple backticks.)*
...
1. Click delete on thread → confirmation dialog
...
- Editor canvas should dominate — minimal chrome
...
Each phase is broken into tasks that should be completable as discrete units of work. Tasks within a phase are ordered — later tasks may depend on earlier ones.
...
**1.2 — Basic TipTap editor**
Get a TipTap editor rendering in the window with all the formatting from the WYSIWYG table above working via keyboard shortcuts. Use hardcoded content for now — no file I/O yet.
*Done when:* Editor renders styled content. All formatting shortcuts work.

**1.3 — Markdown → editor parser**
...
**1.4 — Editor → markdown serializer**
...
**1.6 — Code block syntax highlighting**
...
**2.2 — Comment marker parsing and serialization**
...
**2.3 — Comment highlight rendering**
...
**2.5 — Comment creation flow**
Wire the full flow: select text → `Cmd+Shift+M` → marker created → sidebar input focused → submit or cancel. On save, markers are persisted to the `.md` file and thread data to the JSON sidecar.
*Done when:* Can create a comment, save, close, reopen, and see the comment intact. Cancel removes all traces.

**2.6 — Reply, resolve, delete, navigate**
...
- **Round-trip:** `serialize(parse(fixture))` ≈ `fixture` for the test fixture in the spec.
...
- Open a `.md` file → verify content renders correctly
- Edit → save → reopen → verify persistence
- Create comment → verify highlight + sidebar → save → reopen → verify persistence
...
**Generated companion** bridges the gap for non-Gutter users — anyone can read the comment threads in a standard markdown viewer or GitHub PR.
...
│   ├── src/
│   │   ├── main.rs
│   │   ├── commands/        # Tauri commands (file I/O, comments, workspace)
│   │   └── companion.rs     # .comments.md generator
│   ├── Cargo.toml
│   └── tauri.conf.json
├── src/                     # React frontend
│   ├── App.tsx
│   ├── main.tsx
│   ├── components/
│   │   ├── Editor/          # TipTap editor, extensions, markdown parser/serializer
│   │   ├── Comments/        # Comments panel, threads, input
│   │   ├── FileTree/        # File tree sidebar
│   │   ├── StatusBar.tsx
│   │   └── CommandPalette.tsx
│   ├── stores/              # Zustand stores (editor, comments, workspace)
│   ├── hooks/               # Custom hooks for file ops, comments
│   ├── styles/              # Editor CSS, theme variables
...
1. **Author identity** — MVP: configurable name in settings. Future: integrate with git config.
2. **File portability** — Renaming/moving a `.md` file should move the sidecar and companion with it. Enforce via app, or rely on the naming convention for manual moves.
